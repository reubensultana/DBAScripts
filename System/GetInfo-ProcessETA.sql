...
SELECT 
    ...
    CONVERT(varchar(20), DATEADD(ms, er.[estimated_completion_time], CURRENT_TIMESTAMP), 20) AS [Estimated Completion Time],
    ...
    CONVERT(varchar(1000), (
        SELECT SUBSTRING(est.[text], er.[statement_start_offset]/2, 
            CASE WHEN er.[statement_end_offset] = -1 THEN 1000 ELSE (er.[statement_end_offset] - er.[statement_start_offset])/2 END)
